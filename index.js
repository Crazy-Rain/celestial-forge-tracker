// Celestial Forge Tracker v8.1 - MERGED WORKING VERSION
    ...
    <div id="celestial-forge-settings" class="extension-settings">
        <div class="inline-drawer">
            <div class="inline-drawer-toggle inline-drawer-header">
                <b>⚒️ Celestial Forge Tracker v8.1</b>
...
function updatePerkListUI() {
    ...
    const html = state.acquiredPerks.map(perk => {
        ...
        // ENHANCED SCALING UI with progress bar!
        const scalingInfo = perk.scaling 
            ? `<div style="margin-top:4px;padding:4px;background:rgba(46,204,113,0.1);border-radius:3px">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:2px">
                    <span style="color:#2ecc71;font-size:10px;font-weight:bold">
                        ⬆️ Level ${perk.scaling.level}${perk.scaling.uncapped ? '/∞' : `/${perk.scaling.maxLevel}`}
                        ${perk.scaling.uncapped ? ' 🔓' : ''}
                    </span>
                    <span style="color:#2ecc71;font-size:9px">
                        ${perk.scaling.xp}/${perk.scaling.xp_needed} XP
                    </span>
                </div>
                <div style="background:#222;border-radius:4px;height:6px;overflow:hidden">
                    <div style="background:linear-gradient(90deg,#2ecc71,#27ae60);height:100%;width:${Math.min(100, (perk.scaling.xp / perk.scaling.xp_needed) * 100)}%;transition:width 0.3s ease"></div>
                </div>
               </div>`
            : '';
        ...
        return `<div style="padding:5px;margin:3px 0;border:1px solid var(--SmartThemeBorderColor);border-radius:3px;font-size:11px;background:rgba(233,69,96,0.1)">
            <div style="display:flex;justify-content:space-between;align-items:center">
                ...
                <button class="menu_button forge-remove-perk" data-id="${perk.id}" style="padding:2px 5px;font-size:10px">×</button>
            </div>
            <div style="color:#888;font-size:10px;margin-top:2px">${perk.description || 'No description'}</div>
            <div style="color:#666;font-size:9px;margin-top:2px">[${perk.flags.join(', ') || 'No flags'}]</div>
            ${scalingInfo}
        </div>`;
...
jQuery(async () => {
    ...
    log('✅ Celestial Forge Tracker v8.1 loaded!');
